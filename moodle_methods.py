...
def create_new_user():
    ...
    driver.find_element(By.CSS_SELECTOR, "input#id_department").send_keys(locators.dept)
    sleep(0.25)
...
def delete_a_new_user():
    driver.find_element(By.XPATH, f'//span[contains(.,"Site administration")]').click()
    sleep(0.30)
    assert driver.find_element(By.LINK_TEXT, 'Users').is_displayed
    driver.find_element(By.LINK_TEXT, 'Users').click()
    sleep(0.30)
    ...
    if driver.find_element(By.ID, 'fgroup_id_email_grp_label') and \
        ...
        if driver.find_element(By.XPATH, f'//td[contains(.,"{locators.email}")]'):
            ...
            print(f'User is deleted with {locators.email}')
        else:
            print(f'Unable to delete user {locators.email}')
